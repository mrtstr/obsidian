...
### asymptotically bias
- given a [[statistical estimator]] $T_n$ is estimates based in $n$ samples
- the asymptotical bias of $T_n$ is the [[bias]] that would with sa sample size $n$ that is converging against infinity

$$
\begin{split}
\mathbb{B}_T[\vartheta] 
&= \lim_{x \to \infty} \mathbb{E}_\vartheta[T_n] - \tau(\vartheta) \\
&= \lim_{x \to \infty} \mathbb{E}_{X \sim \mathbb{P}_\vartheta}\left[T(X_1, ..., T_n)\right] - \tau(\vartheta) \\
\end{split}
$$

- since the bias is already about sampling infinite times the only difference between the asymptotical bias and the bias is the asymptotical bias ignores difivations that are converging againt zero 
#### example
- the following estimator is asymptotically unbias but not unbiased 
...
Basic
 asymptotically bias
 - definition
 - interpretation: difference to bias
 - example for an estimator that is biased but asymptotically unbiased
