...
- with the [[eigendecomposition]] of the empirical [[covariance matrix]] $\hat\Sigma = U\Lambda U^\top$  where $\Lambda = \mathrm{diag}(\lambda_1, \dots, \lambda_d)$ contains the [[eigenvalue]] of $\hat\Sigma$, and $U$ is an [[orthonormal]] basis of [[eigenvector]].
- the [[risk|excess risk]] can be decomposed as follows

- the parameters $\theta^* = U\beta$, where $\beta = U^\top \theta^*$ can be expressed and the eigenbasis of $U$ → $\beta$ is the parameter vector in the coordinates of the eigenbasis 
...
START
Basic
[[eigendecomposition]] of the excess risk of the [[ridge regression]]
