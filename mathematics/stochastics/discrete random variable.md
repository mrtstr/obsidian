A [[random variable]] in a [[probability space]] is a mapping from the [[sample space]] to a [[measurable space]] (often real numbers). A [[discrete random variable]] is a [[random variable]] that can take only a finite number of values or at most infinite but [[countable]] number of values.
...
Given all possible [[set|sets]] of real values $C \subseteq \mathbb{R}$. Each [[set]] $C$ defines a [[event]] in the original [[probability space]].  The [[discrete distribution|distribution]] of a [[discrete random variable]] is [[set]] of the [[probability|probabilities]] of these [[event|events]].
$$
...
The probability that the [[discrete random variable]] $X$ is inside the [[set]] of real number $C \subseteq \mathbb{R}$ is equal to the sum of the probabilities of its set elements $x_i \in C$
$$
