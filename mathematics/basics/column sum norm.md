...
$$
### proof that the [[column sum norm]] is a [[norm]]
1) [[positive definite]]
(if the maximum is zero everything else has to be zero too)
$$
\begin{split}
|||A||_{1\rightarrow 1} &= \max_{j \in [m]} \sum_{i \in [n]} |a_{ij}| = 0 \\
\Leftrightarrow A &= 0
\end{split}
$$
...
$$
\begin{split}
||A \cdot \lambda ||_{1\rightarrow1} 
&=  \max_{j \in [m]} \sum_{i \in [n]} |a_{ij}\cdot \lambda | \\
&= |\lambda | \max_{j \in [m]} \sum_{i \in [n]} |a_{ij}| \\
&= |\lambda| \cdot ||A ||_{1\rightarrow1}  \\
\end{split}
$$
...
\begin{split}
||A + B||_{1\to1}
=& \max_{j \in [m]} \sum_{i \in [n]} |a_{ij}+b_{ij}| \\
\leq& \max_{j \in [m]} \sum_{i \in [n]} |a_{ij}| + |b_{ij}| \\
\leq& \max_{j \in [m]} \sum_{i \in [n]} |a_{ij}| + \max_{j \in [m]} \sum_{i \in [n]} |b_{ij}| \\
\leq& ||A||_{1\to1} + ||B||_{1\to1}\\
