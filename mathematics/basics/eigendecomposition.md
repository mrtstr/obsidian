...
- existence
- properties
- how does it look like then the matrix doesn't have a full [[rank]]?
Back: 
...
- note: the reverse is not true i.e. if $A$ doesn't have have distinct eigenvalues it can still have $n$ [[linear independent]] [[eigenvector]] and thus still be a [[eigendecomposition]]
###### proof
- let $\lambda_1, ..., \lambda_n$ be the [[eigenvalue]] of $A$ with the corresponding [[eigenvector]] $v_i$ $\Rightarrow Av_i = \lambda_iv_i$
- since $v_1,..., v_n$ are [[linear independent]] $B=(v_1, ..., v_n)$ is invertable
