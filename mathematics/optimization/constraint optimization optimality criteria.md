...
### stationary point
$\bar x$ is a **stationary point** if
1) $\bar x \in \mathcal{F}$ is [[feasible set|feasible point]]
2) $\mathcal{L}\left(\mathcal{F}, x^*\right) = \mathcal{T}\left(\mathcal{F}, x^*\right)$ a [[constraint qualification]] holds in $\bar x$
3) $\forall d \in \mathcal{L}\left(\mathcal{F}, x^*\right): \nabla f\left(x^*\right)^\top d \geq 0$ there are no decent directions in the [[linearized cone]]
...
START
Basic
stationary point for [[constraint optimization problem]]
- definition
