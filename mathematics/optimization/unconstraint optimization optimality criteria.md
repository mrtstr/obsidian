...
- there are necessary optimality of first second (and higher) order, but they can only rule out candiates
- to prove that a point is a [[local minimum]] we need a sufficient optimality condition of second order but not every [[local minimum]] satisfies them
→ that means that there is a gap between the rules that contains points that cannot be ruled out nor confirmend as a [[local minimum]] but there points are rare 
...
START
Basic
summary: unconstraint optimization optimality theory
