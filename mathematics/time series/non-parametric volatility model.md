...
### problem with high frequency data
- in reality the price is not known exactly because of the bid ask spread thus we have noise on the observations

$$
Y_{t,n} = X_{t,n} + \eta_{t,n}
$$
- for high frequency data thus $n \to \infty$ the estimator for the realized volatility is dominated by the noise $\sum_{t=1}^n \eta_{t,n}^2$ 

$$
\begin{split}
RV
&= \sum_{t=1}^n Y_{t,n}^2  \\
&= \sum_{t=1}^n X_{t,n}^2 + \sum_{t=1}^n \eta_{t,n}^2 + + 2\sum_{t=1}^n \eta_{t,n}X_{t,n} \\
\end{split}
$$
...
START
Basic
[[non-parametric volatility model]]: problem with high frequency data
