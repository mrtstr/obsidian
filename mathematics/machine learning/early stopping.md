...
- looking at the update rule of the [[quadratic loss model]] we see that the size of the weights is increasing as we reach the optimum $\widehat{\mathcal{w}}$
- stopping early has a regularizing effect comparable to [[l2 regularization]]
...
- if we stop early after $t$ steps we have the equivalent effect that we have with [[l2 regularization]] with a factor of $\gamma$ and a learning rate of $\alpha$
...
![[quadratic loss model#update rule of the quadratic loss model]]
...
- algorithm
- update rule of the [[quadratic loss model]] with reason why early stopping has a regularizing effect
- in which step we have to stop to have a similar effect as [[l2 regularization]] with factor $\gamma$ and learning rate $\alpha$
...
START
Basic
given the update rule of the [[quadratic loss model]] and the optimizer of the [[l2 regularization|l2 regularized optimizer]] $\mathcal{w}_\gamma$
- show that [[early stopping]] has a similar effect to [[l2 regularization]] when stopping in period $t \approx \frac{1}{\gamma \alpha}$
