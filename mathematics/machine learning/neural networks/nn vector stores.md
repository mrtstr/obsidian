...
- problem: since there are a lot of vectors stored, checking each would be $\mathcal{O}(N)$ which is too slow → **approximated k nearest neighbor search**
...
- search the graph greedy and keep the similarity scores and stop when local minimum found
...
START
Basic
[[nn vector stores#Hierarchical Navigable Small World Graph (HNSW)]]
...
START
Basic
[[nn vector stores#Inverted File Index (IVF)]]
- concept
- what is it used for

[[nn vector stores#Quantization-Based Indexes (Quantized)]]
- concept
- what is it used for
- two versions
Back: 
