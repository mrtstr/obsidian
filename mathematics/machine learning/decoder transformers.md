...
- training procedure
- inference procedure
- weight sharing: concept and pros/cons
...
- If the user input is a sequence of length $S$ the output would also be of length $S$, but we would just need the last element and ignore the rest
- then we pass the user input plus the first generated token in the model to generate the second token and repeat that process until the full response is generated
- note that we don't have to recalculate the output for the whole user input because we can cache the logits for efficiency
...
Basic
[[decoder transformers]]
- how does mapping the signals from the model dimension back to the dimension of the vocabulary work? Give two options with pros and cons.
