...
- but quadratic with the model dimension $d_\mathrm{model}$ and in the [[self attention layer]] also with the sequence length $S$

### hardware for LLM training
- only feasible on GPUs
- **parallel floating point units:** GPU: 10k+, CPU: number of cores
- **inter device bandwidth**: GPU: 1 TB/s, CPU: 10-30 GB/s
- **memory bandwidth**: GPU: multiple TB/s, CPU: 50-100 GB/s
- GPUs include **Tensor Cores** = specialized hardware, designed to accelerate matrix-multiply-accumulate operations

- problems
	- **batch sizes are too large** to fit on single GPU → **data parallelism**
	- **models are too deep** to fit on a single GPU → **pipeline parallelism**
	- **single layers are too large** to fit in a single GPU → **tensor parallelism**

![[Pasted image 20251124170528.png]]

#### data parallelism
- *Copy the whole model on every GPU, split the data*
- divide batches in micro batches 
- each GPU calculates the gradients for a single micro batch
- then accumulate the gradients and update the weights

![[Pasted image 20251124170322.png]]
#### pipeline parallelism
- *split the model layers into stages across GPUs, send activations through a pipeline*
- problems: **scheduling is more complex** and **communication overhead**

![[Pasted image 20251124170459.png]]

#### tensor parallelism
- *split big weight matrices/tensors across GPUs, work together on each layer*
...
![[decoder transformers#decoder transformers]]
...
- which number format is used for LLM training and why?
Back: 
...
START
Basic
cost of [[scaling of LLM training]] in FLOPs
...
### concept
- [[neural network]] for generation of content (often text) using [[transformer block|transformer blocks]] (e.g. GPT)
...
### architecture
0) [[tokenization]] → token vector $x \in \mathbb{N}^{B \times S}$
1) [[embedding layer]] → dense embedding vector $X \in \mathbb{R}^{B \times S \times D}$
2) multiple [[transformer block]] → internal dense representation $X_m \in \mathbb{R}^{B \times S \times D}$
3) final [[norm layer]] + [[nn linear layer]] → [[logit function|logits]] $Z = \mathrm{LL}\left(\mathrm{LN}\left(X_m\right)\right) \in \mathbb{R}^{B \times S \times V}$
...
- for the output linear transformation mapping back to $\mathbb{R}^{B \times S \times D} \to \mathbb{R}^{B \times S \times V}$, there are two options 
	- reuse the [[embedding layer|embedding matrix]]
	- training a separate matrix
- reusing the [[embedding layer|embedding matrix]] **reduces parameters**, **improves generalization** and **encourages symmetry** between encoding and decoding but reduces flexibility between input and output space
### training

- a text as training data is tokenized (see [[tokenization]]) to a sequence of token indices  $x \in \mathrm{R}^S$ and the lables $y \in \mathrm{R}^S$ which are a shifted version of $x$ by one position
...
- after the [[embedding layer|embedding layer]] we have input data $X$ and the causal mask $M$ that should prevent that information about tokens not known at the time are used
...
- the Input [[embedding layer]] $X \in \mathbb{R}^{B \times S \times D}$ represents a sequence of token embeddings/features of the length $S$ with each feature/embedding being represented in the $\mathbb{R}^{D}$
...
START
Basic
hardware aspects for LLM training
- GPUs vs CPU (4 reasons why GPUs are the only option)
- 3 scalability limitations and solutions
- additional way to make the distributed training more efficient
Back: 
...
![[Pasted image 20251124170538.png]]
#### data parallelism
- *Copy the whole model on every GPU, split the data*
- divide batches in micro batches 
- each GPU calculates the gradients for a single micro batch
...
![[Pasted image 20251124170315.png]]
#### pipeline parallelism
- *split the model layers into stages across GPUs, send activations through a pipeline*
- problems: **scheduling is more complex** and **communication overhead**

![[Pasted image 20251124170447.png]]
#### tensor parallelism
- *split big weight matrices/tensors across GPUs, work together on each layer*

![[Pasted image 20251124170417.png]]
...
START
Basic
summary [[scaling of LLM training]]
- batch size
- what impacts the required flops to train a model
- hardware: 4 techniques to scale and speedup the training
- number format used
Back: 
## LLM training
####  batch size
