...
- the [[derivative]] of the loss with respect to the $j$ element of the output of a layer $\delta^{(l)}_j$ can be expressed as a product of the derivative of its [[activation function]] multiplied by a weighted sum of the $\delta^{(l-1)}$ of the previous layer
...
- this can be decomposed to the outer [[derivative]] of the [[loss function]] with respect to the activation of the $k$ layer $\mathrm{a}^{(k)}$ and the [[derivative]] of the [[activation function]] $g^{(k)}$
...
- we have a batch $\mathcal{S}^B$ with index set $B \subset \{1, ..., m\}$ size $s=|B|$

$$
\mathcal{S}^B = \left\{\left(x^{(B_1)}, y^{(B_1)}\right), ..., \left(x^{(B_s)}, y^{(B_s)}\right)\right\}
$$

- for each batch we have a design matrix $X^B$ and label matrix $Y^B$

$$
\begin{split}
X^B &= \left[x^{(B_1)}, ..., x^{(B_s)}\right] \in \mathbb{R}^{n_0 \times s} \\
Y^B &= \left[y^{(B_1)}, ..., y^{(B_s)}\right] \in \mathbb{R}^{n_L \times s} \\
\end{split}
$$
...
$$
\begin{split}
\mathrm{Z}^{(i)} &= \underbrace{W^{(i)}}_{n_i \times n_{i-1}} \underbrace{\mathrm{A}^{(i-1)}}_{n_{i-1} \times s} + \underbrace{b^{(i)}}_{n_i \times 1} \underbrace{\mathbb{1}_s^\top}_{1 \times s} \in \mathbb{R}^{n_i \times s}  \: \text{with} \: & b^{(i)} \mathbb{1}^\top = [b^{(i)}, ..., b^{(i)}]  \\
\mathrm{A}^{(i)} 
&= g^{(i)}\left(\mathrm{Z}^{(i)}\right) \in \mathbb{R}^{n_i \times s}  \\
\end{split}
$$
...
\Delta^{(L)}
&= 
A^{(L)} - Y^{(L)}  \in \mathbb{R}^{n_L \times s} \\
\nabla_ {W^{(L)}} \mathcal{R} &= \frac{1}{s} \underbrace{ \Delta^{(L)}}_{n_L \times s} \underbrace{A^{(L-1)\top}}_{s \times n_{L-1}}  \in \mathbb{R}^{n_L \times n_{L-1}} \\
\nabla_ {b^{(L)}} \mathcal{R} 
&= \frac{1}{s} \underbrace{ \Delta^{(L)}}_{n_L \times s} \underbrace{\mathbb{1}_s}_{s \times 1} \in \mathbb{R}^{n_{L}} \\
\end{split}
$$
...
\Delta^{(k)}
&= 
\underbrace{g^{(k)'}\left(Z^{(k)}\right)}_{n_k \times s} \odot \underbrace{W^{(k+1)}}_{n_{k+1} \times n_k} \underbrace{\Delta^{(k)}}_{n_{k} \times s}  \in \mathbb{R}^{n_L \times n_{L-1}} \\
\nabla_ {W^{(L)}} \mathcal{R} &= \frac{1}{s} \underbrace{ \Delta^{(k)}}_{n_k \times s} \underbrace{A^{(k-1)\top}}_{s \times n_{k-1}}  \in \mathbb{R}^{n_k \times n_{k-1}} \\
\nabla_ {b^{(k)}} \mathcal{R} 
&= \frac{1}{s} \underbrace{ \Delta^{(k)}}_{n_k \times s} \underbrace{\mathbb{1}_s}_{s \times 1} \in \mathbb{R}^{n_{k}} \\
\end{split}
$$
...
START
Basic
[[backpropagation]] summary: definitions and solutions for the following
- calculate  $\nabla_ {W^{(k)}} l$ and $\nabla_ {b^{(k)}} l$
- $\delta^{(k)}$ and $\delta^{(L)}$
...
### elementwise product
- given two [[matrix|matrices]] with the same shape $A, B \in \mathbb{R}^{n \times m}$
